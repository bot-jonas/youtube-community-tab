...
class PostsIterator:
    ...
    def _get_initial_posts_data(self):
        resp = get_requests_handler().get(self.channel.community_url)

        if resp.status_code != 200:
            raise Exception(f"Could not get data using the url `{self.channel.community_url}`")
        ...
        try:
            ...
            raise Exception("Could not parse ytInitialData json")
        ...
        body = {
            "context": {
                "client": {
                    ...
                    "originalUrl": self.channel.community_url,
    ...
    def _append_posts_from_items(self, items):
        ...
        for item in items:
            ...
            if kind == "backstagePostThreadRenderer":
                self._posts.append(Post.from_data(item[kind]["post"]))
    ...
    def _get_community_tab(self, ytInitialData):
        ...
        raise Exception("Could not find community tab in the ytInitialData")
    ...
    def _get_posts_data_from_community_tab(self, community_tab):
        try:
            ...
            raise Exception("Could not get posts data from the community tab")
    ...
    def _update_session_attributes(self, community_tab, ytInitialData):
        try:
            ...
            raise Exception("Could not get the click tracking params from the community tab")

        try:
            self._visitor_data = ytInitialData["responseContext"]["webResponseContextExtensionData"]["ytConfigData"]["visitorData"]
        except KeyError:
            raise Exception("Could not get the visitor data from the ytInitialData")
